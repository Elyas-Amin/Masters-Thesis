...
def _partial_ordering(elements, shuf=True, reverse=False):
    ...
    >>> all(f(x, _partial_ordering(x)) == sorted(x) for _ in range(10))
    True
    """
    # Define comparison function for sorting with random tie-breaking
    if not shuf:
        def my_cmp(x, y):
            return (x > y) - (x < y)  # Python 3 replacement for cmp
    else:
        def my_cmp(x, y):
            if x != y:
                return (x > y) - (x < y)
            else:
                return random.choice([-1, 1])  # Random tie-breaking

    # Sort the elements with the custom comparison function
    sorted_indices = sorted(enumerate(elements), 
                            key=functools.cmp_to_key(lambda x, y: my_cmp(x[1], y[1])),
                            reverse=reverse)
    
    # Return the sorted indices (partial ordering)
    return tuple(i for i, _ in sorted_indices)
...
def find_least_common(T):
    ...
    counter = Counter(itertools.chain.from_iterable(itertools.chain.from_iterable(S for S in D) for D in T))
    
    if not counter:
        return frozenset(), 0
    
    # Sort and find the least common elements
    sorted_pairs = counter.most_common()  # This returns a list of (element, count) tuples, sorted by count
    n = sorted_pairs[-1][1]  # Get the count of the least common element (which is the last in the sorted list)
    
    # Return a frozenset of elements with the least common count and the count
    return frozenset(u for u, c in itertools.takewhile(lambda x: x[1] == n, reversed(sorted_pairs))), n
